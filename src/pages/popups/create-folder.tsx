...
const CreateFolder: Component<CreateFolderProps> = (props) =>{
    ...
    const [state, setState] = createSignal<{ loading: boolean, message?: string, errors?: { folderName: string } }>({ loading: false });
    ...
    const submit = async() =>{
        ...
        try{
            if(input().length > 0){
                ...
                setState({ loading: false, errors: { folderName: "Folder name most have atleast one character" } })
            }
        }catch(error){
            console.error(error);
    ...
    return (
        <PopUp>
            <div style={{ "width": "20rem", display: "flex", "flex-direction": "column", "align-content": "center", gap: "0.8rem" }}>
                <FormInput label="Create Folder" error={state().errors?.folderName} valueChange={setInput} value={input()} placeholder="Folder name"  type="text" name="fileName"/>
                <div>
