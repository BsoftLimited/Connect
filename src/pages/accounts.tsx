...
interface AccountOptionProps{
    label: string,
    pageOption?: { page: Pages, chosen: (page: Pages) => void },
    current: Pages,
...
const AccountOption: ParentComponent<AccountOptionProps> = (props) =>{
    const clicked = () =>{
        if(props.clicked){
            props.clicked();
        }else if(props.pageOption){
            props.pageOption.chosen(props.pageOption.page);
        }
    }

    return (
        <div onClick={clicked} class={`account-options ${props.current === props.pageOption?.page ? "active" : "clicakble"}`}>
            {props.children}
            ...
            <div style={{ display: "flex", width: "100%", flex: 1, "flex-direction": "row", overflow: "hidden", padding: "2rem 3rem 3rem 3rem" }}>
                <div style={{ display: "flex",  "flex-direction": "column"}} >
                    <div style={{ display: "flex",  "flex-direction": "column", flex: 1, gap: "1rem", padding: "1rem" }} >
                        <AccountOption label={ "Profile" } pageOption={{ page: "profile", chosen: setPage}} current={page()}>
                        ...
                        <Show when={userState().user?.users}>
                            <AccountOption label={ "Accounts" } pageOption={{ page: "users", chosen: setPage}} current={page()}>
                        ...
                        <AccountOption label={ "Settings" } pageOption={{ page: "settings", chosen: setPage}} current={page()}>
                    ...
                    <div style={{ display: "flex",  "flex-direction": "column", gap: "2rem", padding: "1rem" }} >
                        <AccountOption label={ "Logout" } current={page()}>
