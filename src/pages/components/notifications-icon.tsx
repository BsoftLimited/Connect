...
const NotificationIcon: Component<NotificationIconProps> = (props) =>{
    const { sessionState } = useUserContext();

    const unseen = () =>{
        return sessionState().data!.notifications.filter((init)=> !init.seen).length;
    }

    return (
        <Show when={sessionState().data?.config.notifications === true}>
            <span class="clicakble" onClick={props.open}>
