...
const Streaming = () =>{
    const { appState, closeStream, pageState } = useAppContext();
    ...
    const download = () =>{
        window.location.href = `/download/${appState().directory!.path.replaceAll("\\", "/")}/${pageState().params!}`;
    }
    ...
    return (
        <div style={{  flex: 1, width: "100%", overflow: "hidden", background: "black" }}>
            <video class="video" autoplay poster="/assets/images/music-poster.png" controls={false} ref={videoElement} onPlaying={onVideoChange} onVolumeChange={onVideoChange} onPause={onVideoChange} onTimeUpdate={onVideoChange} muted={streamingState().mute} src={`/files/${appState().directory!.path}/${pageState().params}`}></video>
                ...
                <div class="controls">
                    <div style={{ padding: "1rem", display: "flex", "flex-direction": "row", width: "100%", gap: "2rem" }}>
                        <div style={{ flex: 1, display: "flex", "flex-direction": "row", "align-items": "center", gap: "0.8rem" }}>
                            ...
                            <h3 style={{ color: "whitesmoke", "font-weight": "lighter" }}>Playing : {pageState().params}</h3>
                        </div>
