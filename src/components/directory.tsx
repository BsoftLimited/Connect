import { createSignal, Match, onMount, Show, Switch, For } from "solid-js";
...
const Directory = () => {
    ...
    const [fileContextStatus, setFileContext] = createSignal<ContextStatus>({ show: false });
    const [filter, setFilter] = createSignal("");

    onMount(()=> {
        document.addEventListener("click", ()=> setFileContext({ show: false }));
    });

    const fileContext = (file: DirectoryFile, x: number, y: number) =>{
        setFileContext({ show: false });
        setFileContext({ show: true, x: `${x}px`, y: `${y}px`, file });
    }
                    ...
                    <div style={{ width: "100%", flex: 1, overflow: "auto", padding: "0.5rem" }}>
                        <div id="files-container" class="files-container">
                            <For each={state().directory!.files.filter((file)=> file.name.includes(filter()))} fallback={<div>No items</div>}>
                                {(item) => <FileView  file={item} onContext={(x, y)=>fileContext(item, x, y) } />}
                            </For>
                        </div>
                    </div>
                    <Show when={fileContextStatus().show}>
                        <FileContextMenu x={fileContextStatus().x} y={fileContextStatus().y}  file={fileContextStatus().file}/>
                    </Show>
                </div>
            </Match>
        </Switch>
